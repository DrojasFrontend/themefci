<?php 
/*
    Template Name: Educación continua y eventos
*/
...
">
					...
					<div>
						<input type="text" name="search" class="form-control" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>" placeholder="Busca por palabra clave">
					</div>
					<div>
						<input type="date" id="custom-date-input" class="form-control date-p" name="custom_date" value="<?php echo (isset($_GET['custom_date'])) ? $_GET['custom_date'] : ''; ?>" placeholder="Fecha">
					</div>
					<div>
						<select id="type_especialidades" name="type_especialidades" class="form-select">
							<option value="<?php  echo !empty($_GET['type_especialidades']) ? $_GET['type_especialidades'] : '';  ?>"><?php  echo !empty($_GET['type_especialidades']) ? $_GET['type_especialidades'] : 'Especialidades';  ?></option>
							<?php 
							$opciones_categorias_eventos = get_field_choices('categorias_eventos',true);
							foreach ($opciones_categorias_eventos[1]["choices"] as $opcion) {
								?>
                    ...
                    <div class="educacontigeneral__cadaresult__content">
                        ...
                        <div class="educacontigeneral__cadaresult__content__content">
                            ...
                            <div class="educacontigeneral__cadaresult__content__cta">
                                <a href="<?= $cadaPublicacion["enlace"] ?>" class="btn btn_vermas btn-azul">Ver evento</a>
                            </div>
							<?php if($cadaPublicacion["memorias"] != ''){ ?>
							<div class="educacontigeneral__cadaresult__content__cta mt-3">
                                <a href="<?= $cadaPublicacion["memorias"] ?>" class="btn btn_vermas">Memorias</a>
                            </div>
<?php } ?>
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  limpiarBtn.addEventListener("click", function() {
    ...
    window.location = "/educacion-continua-eventos/";
  });
});
</script>
<?php  get_footer(); ?>
